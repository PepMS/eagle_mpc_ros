...
class MpcController():
    def __init__(self, trajectoryPath, trajectoryDt, trajectorySolver, mpcPath, mpcType):
        ...
        if mpcType == 'Rail':
            self.mpcController = multicopter_mpc.RailMpc(solver.xs, self.trajectory_dt, mpcPath)
        elif mpcType == 'Weighted':
            self.mpcController = multicopter_mpc.WeightedMpc(trajectory, self.trajectory_dt, mpcPath)
        else:
            self.mpcController = multicopter_mpc.CarrotMpc(trajectory, solver.xs, self.trajectory_dt, mpcPath)
