...
void MpcRunner::initializeParameters()
{
    ...
    nh_.param<std::string>(ros::this_node::getNamespace() + "/trajectory_config", node_params_.trajectory_config_path,
                           "");
    node_params_.trajectory_config_path = eagle_mpc::getYamlPath(node_params_.trajectory_config_path);
    ...
    node_params_.mpc_config_path = eagle_mpc::getYamlPath(node_params_.mpc_config_path);
...
void MpcRunner::initializeMpcController()
{
    ...
    ROS_WARN("Trajectory dt: %ld", node_params_.trajectory_dt);
...
void MpcRunner::initializeVariables()
{
    ...
    fb_gains_       = Eigen::MatrixXd::Zero(mpc_controller_->get_actuation()->get_nu(),
                                      mpc_controller_->get_robot_state()->get_ndx());
...
void MpcRunner::initializeSubscribers()
{
    ...
    if (node_params_.arm_enable) {
        subs_joint_state_ = nh_.subscribe("/joint_states", 1, &MpcRunner::callbackJointState, this,
...
void MpcRunner::initializePublishers()
{
    ...
    if (node_params_.arm_enable) {
        ...
        for (std::size_t i = 0; i < n_joints; ++i) {
            pub_arm_commands_.push_back(
                nh_.advertise<std_msgs::Float64>("/joint_command_" + std::to_string(i + 1), 1));
        }
    }
}
